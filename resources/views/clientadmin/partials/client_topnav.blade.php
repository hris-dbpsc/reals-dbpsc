<nav class="topnav navbar navbar-expand shadow justify-content-between justify-content-sm-start navbar-light bg-white" id="sidenavAccordion">
    ...
    <a class="navbar-brand pe-3 ps-4 ps-lg-2" href="{{ route('clientadmin_dashboard') }}">REALS - DBPSC</a>
    <!-- Navbar Items-->
    <ul class="navbar-nav align-items-center ms-auto">
        <!-- User Dropdown-->
        <li class="nav-item dropdown no-caret dropdown-user me-3 me-lg-4">
            <a class="btn btn-icon btn-transparent-dark dropdown-toggle" id="navbarDropdownUserImage" href="javascript:void(0);" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                <img class="img-fluid" src="{{ Auth::guard('clientadmin')->user()->photo ? asset('assets/users/clientadmin/' . Auth::guard('clientadmin')->user()->photo) : asset('assets/assets/img/demo/user-placeholder.svg') }}" />
            </a>
            <div class="dropdown-menu dropdown-menu-end border-0 shadow dropdown-menu-animation" aria-labelledby="navbarDropdownUserImage">
                <h6 class="dropdown-header d-flex align-items-center">
                    <img class="dropdown-user-img" src="{{ Auth::guard('clientadmin')->user()->photo ? asset('assets/users/clientadmin/' . Auth::guard('clientadmin')->user()->photo) : asset('assets/assets/img/demo/user-placeholder.svg') }}" />
                    <div class="dropdown-user-details">
                        <div class="dropdown-user-details-name">{{ Auth::guard('clientadmin')->user()->firstname }} {{ Auth::guard('clientadmin')->user()->lastname }}</div>
                        <div class="dropdown-user-details-email">{{ Auth::guard('clientadmin')->user()->email }}</div>
                    </div>
                </h6>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="{{ route('clientadmin_profile', encrypt(Auth::guard('clientadmin')->user()->id)) }}">
