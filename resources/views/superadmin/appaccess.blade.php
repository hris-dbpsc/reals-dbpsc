                                ...
                                <table id="datatablesSimple">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                // Get all applications once (ideally, pass $allApplications from controller)
                                                $allApplications = \App\Models\Applications::all();
                                                // Get this client's access row once
                                                $access = \App\Models\ApplicationsAccess::where('clientid', $client->id)->first();
                                                @endphp
                                                @if($allApplications->isNotEmpty() && $access)
                                                @foreach($allApplications as $application)
                                                @php
                                                $accessField = 'app_' . $application->id;
                                                @endphp
                                                @if(!empty($access->$accessField))
                                                <span class="badge bg-primary mb-1" style="font-size: .8rem; padding: 0.5em 1em;">{{ $application->appname }}</span>    @endif
                                                @endforeach
                                                @endif
                                            </td>
                                            <td>
                                                @php
                                                $hasAccess = \App\Models\ApplicationsAccess::where('clientid', $client->id)->exists();
                                                @endphp
                                                @if($hasAccess && $access)
