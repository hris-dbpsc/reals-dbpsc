                            ...
                            <div class="row align-items-center justify-content-between">
                                <div class="col-auto mt-2">
                                    <h2 class="page-header-title text-body d-flex align-items-center" style="font-size:1.5rem;">
                                        ...
                                        @if($clientId)
                                        @php
                                        $clientObj = $clients->where('id', $clientId)->first();
                                        @endphp
                                        (Client: {{ $clientObj ? $clientObj->clientname : $clientId }})
                                        @elseif($branch)
                                        (Branch: {{ $branch }})
                                        @endif
                                    </h2>
                        ...
                        <div class="card-body">
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                    ...
                                    if ($clientId) {
                                    $filteredUsers = $filteredUsers->where('clientid', $clientId);
                                    } elseif ($branch) {
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex justify-content-center align-items-center">
                                                @php
                                                $userPhotoPath = 'assets/users/users/' . ($user->employeenumber ?? '') . '.jpg';
                                                $userPhotoExists = !empty($user->employeenumber) && file_exists(public_path($userPhotoPath));
                                                @endphp
                                                <img class="img-fluid rounded-circle" style="width: 48px; height: 48px; object-fit: cover;" src="{{ $userPhotoExists ? asset($userPhotoPath) : asset('assets/assets/img/demo/user-placeholder.svg') }}" alt="User Photo" />
                                            </div>
                                        </td>
